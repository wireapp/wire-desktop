  ...
  verifyPinning(hostname, certificate) {
    ...
    for (const pin of pins) {
      ...
      if (url.test(hostname.toLowerCase().trim())) {
        result.verifiedIssuerRootPubkeys = issuerRootPubkeys.length > 0
          ? issuerRootPubkeys.some(pubkey => rs.X509.verifySignature(issuerCertHex, rs.KEYUTIL.getKey(pubkey)))
          : undefined;
