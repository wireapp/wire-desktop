...
node('windows') {
  ...
  stage('Build installers') {
    try {
      withEnv(["PATH+NODE=${NODE}", 'npm_config_target_arch=x64']) {
        ...
        bat 'if not exist "wrap\\dist\\*.msi" (echo Missing MSI package & exit /b 1)'
      }
    } catch (e) {
      currentBuild.result = 'FAILED'
  ...
  stage('Sign installer') {
    if (production || wireGov) {
      ...
      echo 'Skipping signing for non-production build (nightly/custom).'
    }
  }
  ...
  stage('verify') {
    if (production || wireGov) {
      ...
      echo 'Skipping signature verification for non-production build (nightly/custom).'
    }
  }
