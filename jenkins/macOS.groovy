...
node('master') {
  ...
  stage('Build') {
    try {
      ...
      if (production) {
        ...
        sh "productbuild --component 'wrap/build/Wire-mas-x64/Wire.app' '/Applications' --sign '3rd Party Mac Developer Installer: Wire Swiss GmbH (EDF3JCE8BC)' 'wrap/build/Wire.pkg'"
      }
  ...
  stage('Archive build artifacts') {
    if (production) {
      sh "ditto -c -k --sequesterRsrc --keepParent \"${WORKSPACE}/wrap/build/Wire-mas-x64/Wire.app/\" \"${WORKSPACE}/wrap/build/Wire.zip\""
      archiveArtifacts 'wrap/build/Wire.pkg,wrap/build/Wire.zip'
    } else if (custom) {
