  ...
  const showMainWindow = async (mainWindowState: windowStateKeeper.State): Promise<void> => {
    ...
    const options: BrowserWindowConstructorOptions = {
      ...
      webPreferences: {
        ...
        preload: PRELOAD_JS,
        sandbox: false, // Disabled to allow preload-app.js to import shared constants
        webviewTag: true,
      },
                ...
                if (webViewId) {
                  ...
                  try {
                    await LogFactory.writeMessage(
                      message
                        .replaceAll(colorCodeRegex, '$1')
                        .replaceAll(stylingRegex, '')
                        .replaceAll(accessTokenRegex, ''),
                      logFilePath,
                    );
                  } catch (error) {
