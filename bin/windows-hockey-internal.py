...
HOCKEY_APP_ID = os.environ.get('WIN_HOCKEY_ID')
...
if __name__ == '__main__':
    ...
    print 'Uploading Wire for Windows (%s-internal)...' % VERSION
    ...
    data = {
        ...
        'notes': 'New build of Wire for Windows (%s-internal)' % VERSION,
        'notify': 0,
        'status': 2,
    }
    ...
    response = requests.put('%s%s' % (HOCKEY_URL_UPDATE, response.json()['id']), files=files, data=data, headers=headers)
