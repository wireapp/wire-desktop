...
describe('showUnreadCount', () => {
  describe('without tray icon initialization', async () => {
  ...
  describe('with tray icon initialization', async () => {
    it('updates the badge counter and stops flashing the app frame when app is in focus while receiving new messages', done => {
      ...
      await appWindow.loadURL(`file://${path.join(__dirname, '../fixtures/badge.html')}`);
      appWindow.webContents.on('dom-ready', () => {
        assert.strictEqual(appWindow.isFocused(), true);
        assert.strictEqual(appWindow.flashFrame.callCount, 0);
    ...
    it('flashes the app frame when app is not in focus and you receive new messages', async done => {
      const tray = new TrayHandler();
      tray.initTray(TrayMock);
    ...
    it('does change the flash state if the window has already been flashed', async done => {
      const tray = new TrayHandler();
      tray.initTray(TrayMock);
