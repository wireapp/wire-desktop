  ...
  stage('Sign build') {
    try {
      if (production) {
        ...
        bat 'for /r %%i in ("wrap\\build\\*-win32-ia32\\*.exe") do "C:\\Program Files (x86)\\Windows Kits\\10\\bin\\x86\\signtool.exe" sign /t http://timestamp.digicert.com /fd SHA256 /a %%i'
      } else {
  ...
  stage('Sign installer') {
    try {
      if (production) {
        ...
        bat 'for /r %%i in ("wrap\\build\\*-win32-ia32\\*-Setup.exe") do "C:\\Program Files (x86)\\Windows Kits\\10\\bin\\x86\\signtool.exe" sign /t http://timestamp.digicert.com /fd SHA256 /a %%i'
      } else {
